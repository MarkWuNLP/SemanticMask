...
def torch_snapshot_iter(savefun=torch.save,
    ...
    @extension.make_extension(trigger=(10, 'iteration'), priority=-100)
    def torch_snapshot_iter(trainer):
        _torch_snapshot_object(trainer, trainer, filename.format(trainer), savefun)

    return torch_snapshot_iter
...
def torch_load(path, model):
    ...
    if 'snapshot' in path:
        try:
            ...
            model_state_dict_tmp = torch.load(path, map_location=lambda storage, loc: storage)['state_dict']
            model_state_dict = {}
            for key in model_state_dict_tmp:
                model_state_dict[key.replace("module.", "")] = model_state_dict_tmp[key]
                #print(model_state_dict[key.replace("module.", "")].shape)
    else:
...
def parse_hypothesis(hyp, char_list):
    ...
    token_as_list = ["<eos>" if int(idx) >= len(char_list) else char_list[int(idx)] for idx in tokenid_as_list]
