...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        group.add_argument("--transformer-output-layer", type=str, default="embed",
                           ...
                           help='transformer output layer type')
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        if "transformer_output_layer" not in vars(args) or args.transformer_output_layer is None:
            args.transformer_output_layer = "embed"
        # if  args.transformer_output_layer is None:
        #     args.transformer_output_layer = "embed"
    ...
    def target_mask(self, ys_in_pad):
        ...
        return ys_mask.unsqueeze(-2) & m, ys_mask

    def forward(self, xs_pad, ilens, ys_pad, return_acc=False):
        '''E2E forward
        ...
        tgt_mask, ys_mask = self.target_mask(ys_in_pad)
        pred_pad, pred_mask = self.decoder(ys_in_pad, tgt_mask, ys_mask, hs_pad, hs_mask)
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
           ...
           #logging.warning(ys_out_pad)
            #loss_ctc = self.ctc(hs_pad.view(batch_size, -1, self.adim), hs_len, ys_pad)
            logging.warning("CTC loss {0} att loss {1}".format(loss_ctc_data,loss_att_data))
    ...
    def recognize(self, feat, recog_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        for i in six.moves.range(maxlen):
            ...
            if char_list is not None:
                logging.debug(
                    'best hypo: ' + ''.join([ "<eos>" if int(x) >= len(char_list) else char_list[int(x)] for x in hyps[0]['yseq'][1:]]))
