...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument('--dist', default=False, type=strtobool,
                        help='Whether use distributed data parallel for training')
    ...
    parser.add_argument('--fbank-fmax', type=float, default=None,
                        help='')
    parser.add_argument('--n_iter_processes', type=int, default=0,
                        help='')


    ## custom hyper-parameter
    parser.add_argument('--save_iteration', type=strtobool, default=True,
                        help='')
    parser.add_argument('--save_epoch', type=strtobool, default=True,
                        help='')
    parser.add_argument('--use_torchspeech', type=strtobool, default=False,
                        help='')
    parser.add_argument('--idim', default=160, type=int,
                        help='input dimension ')
    parser.add_argument('--maskwords', default=False, type=strtobool,
                        help='whether mask whole word')
    parser.add_argument('--maskratio', default=0.15, type=float,
                        help='mask word ratio')
    parser.add_argument('--odim', default= 9404, type=int,
...
def main(cmd_args):
    ...
    if args.model_module is None:
        model_module = "espnet.nets." + args.backend + "_bacdist: Falsekend.e2e_asr:E2E"
    else:
    ...
    if args.num_spkrs == 1:
        if args.backend == "chainer":
            ...
            if args.use_torchspeech:
                from espnet.asr.pytorch_backend.asr_ts_ddp import train
                train(args)
            else:
                if args.dist:
                    from espnet.asr.pytorch_backend.asr_ddp import train
                    logging.warning("using dist data parallel training, please attend to the batchsize.")
                    train(args)
                else:
